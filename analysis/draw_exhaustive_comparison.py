...
for index, label, col in [(2, r'$\mathit{t_2}$', 'firebrick'), (3, r'$\mathit{t_3}$', 'chocolate'),
    ...
    plt.hlines(experienced_slo, xmin=index - 0.4, xmax=index + 0.4, color=col, linestyle='-',
    ...
    for (processor, consumer) in itertools.product(device_list, device_list):
        ...
        # first_entry = False
    boxplot = ax.boxplot(lines, positions=[index], patch_artist=True, widths=0.36, whis=1.35)
